...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(await prisma.intelItem.findMany({ where: status === "all" ? {} : { status }, orderBy: [{ status: "asc" }, { urgency: "asc" }, { publishedAt: "desc" }, { createdAt: "desc" }] }));
}

export async function POST(req: NextRequest) {
  const validated = validateIntelItem(await req.json().catch(() => ({})));
  if (!validated.ok) return NextResponse.json({ error: validated.error }, { status: 400 });
  return NextResponse.json(await prisma.intelItem.create({ data: validated.data }), { status: 201 });
}
